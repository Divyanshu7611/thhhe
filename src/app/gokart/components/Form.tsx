...
export default function Form({ id, captainId }: { id: string; captainId: string }) {
  ...
  function addMemberInput() {
    if (memberFormRef.current.childElementCount < 29) {
      ...
      node.setAttribute('placeholder', 'Gokart-' + elementCout + ' ID')
  ...
  return (
    <>
      ...
      <section
        ...
        style={{
          backgroundImage: `url("/events/framegokart.png")`,
        ...
        <div className="p-1 grid grid-cols-1 md:grid-cols-2 gap-x-4 items-start">
          ...
          >
            <div className="" ref={memberFormRef}>
              <input type="text" placeholder="Gokart member-1 ID" className="input input-bordered  max-w-xs" />
            </div>
