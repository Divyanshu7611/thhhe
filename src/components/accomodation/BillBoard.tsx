...
//             ))}
//           </div>
//           <Link href={'/accomodation/checkout/payment'} className="absolute btn btn-primary w-full bottom-0">
//             Pay Now
//           </Link>
//         </div>
//       )}
...
export default function BillBoard() {
  ...
  const accomodationBill = billData.registeredEvents.find(bill => bill.id === 'ACCOMODATION')
    ...
    <>
      {load && (
        ...
        >
          ...
          <div className="divider-horizontal"></div>
          <h2 className="">Payment Details</h2>
          <h2 className="font-bold">₹{accomodationBill?.regFee}</h2>

          {accomodationBill && (
            <div className="flex flex-col items-end">
              <div className="flex flex-col items-end text-right">
                <h4 className="text-white/60">{accomodationBill.eventName}</h4>
                <h3>{accomodationBill.regFee}</h3>
              </div>
            </div>
          )}
          <Link href={'/accomodation/checkout/payment'} className="absolute btn btn-primary w-full bottom-0">
