...
export default function Form({ id, captainId }: { id: string; captainId: string }) {
  ...
  function addMemberInput() {
    if (memberFormRef.current.childElementCount < 4) {
      ...
      node.setAttribute('placeholder', 'Robowar member-' + elementCout + ' ID')
  ...
  return (
    <>
      ...
      <section
        ...
        style={{
          backgroundImage: `url("/events/Framerobowar.png")`,
        ...
        <div className="p-1 grid grid-cols-1 md:grid-cols-2 gap-x-4 items-start">
          ...
          >
            <div className="" ref={memberFormRef}>
              <input type="text" placeholder="Robowar member-1 ID" className="input input-bordered  max-w-xs" />
            </div>
